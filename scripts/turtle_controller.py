...
def callback(data):
    ...
    if data.data == "maju":
        twist.linear.x = 2.0  # Kecepatan maju
        twist.angular.z = 0.0
    elif data.data == "mundur":
        twist.linear.x = -2.0  # Kecepatan mundur
        twist.angular.z = 0.0
    elif data.data == "kiri":
        twist.linear.x = 0.0
        twist.angular.z = 2.0  # Kecepatan belok kiri
    elif data.data == "kanan":
        twist.linear.x = 0.0
        twist.angular.z = -2.0  # Kecepatan belok kanan
    else:
        rospy.logwarn(f"Unknown command: {data.data}")
        return

    # Publish gerakan ke /turtle1/cmd_vel
    if pub is not None:  # Pastikan publisher sudah diinisialisasi
        pub.publish(twist)
...
def listener():
    """
    Node listener untuk mendengarkan perintah dari topik /turtle_commands.
    """
    global pub  # Gunakan variabel global untuk publisher
